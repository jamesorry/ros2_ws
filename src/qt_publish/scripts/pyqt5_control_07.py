...
import cv2
from Test_03 import Ui_MainWindow
...
class MainWindow_controller(QtWidgets.QMainWindow):
    ...
    def pose_to_pixel_map(self):
        ...
        center_x = (
            self.robot_1_position['x'] / self.map_resolution) + self.map_center_pixel_x
        center_y = (
            self.robot_1_position['y'] / self.map_resolution) + self.map_center_pixel_y
        center_point = (int(center_x*1.0), int(center_y*(1.0)))
        # print(f"center point_x: {center_x}, center point_y: {center_y}")
        ...
        another_center_x = (
            another_center[0] / self.map_resolution) + self.map_center_pixel_x
        another_center_y = (
            -1.0*another_center[1] / self.map_resolution) + self.map_center_pixel_y
        another_center_point = (int(another_center_x*1.0),
                                int(another_center_y*(1.0)))
        # print(f"another_center point_x: {another_center_x}, another_center point_y: {another_center_y}")
        cv2.arrowedLine(map, center_point, another_center_point,
                        self.point_color_blue)
        # cv2.circle(map, center_point, self.point_size,
        #            self.point_color_blue, self.point_thickness)

        center_x = (
            self.robot_2_position['x'] / self.map_resolution) + self.map_center_pixel_x
        center_y = (
            self.robot_2_position['y'] / self.map_resolution) + self.map_center_pixel_y
        center_point = (int(center_x*1.0), int(center_y*(1.0)))
