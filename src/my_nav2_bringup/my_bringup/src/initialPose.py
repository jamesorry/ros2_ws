...
class NumberPublisherNode(Node):
    ...
    def Publusher_1(self):
        ...
        pose_msg.pose.pose.position.x = 0.0 #0.05    # 0.0     # 0.05
        pose_msg.pose.pose.position.y = 1.0 #0.5     # 1.5     # 0.5
    ...
    def Publusher_2(self):
        ...
        pose_msg.header.stamp = self.get_clock().now().to_msg()
        pose_msg.header.frame_id = "map"
        pose_msg.pose.pose.position.x = 0.0 #0.05       # -1.64   # 0.05
        pose_msg.pose.pose.position.y = -1.5 #-0.5       # 1.47    # -0.5
        pose_msg.pose.covariance[0] = 0.25
        pose_msg.pose.covariance[6 * 1 + 1] = 0.25
        pose_msg.pose.covariance[6 * 5 + 5] = 0.06853891945200942
        pose_msg.pose.pose.orientation.z = 0.0
