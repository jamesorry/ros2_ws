...
{
template <> inline
Pose2D_2 convertFromString(StringView key)
{
...
class Nav2ClientRobot2 : public BT::AsyncActionNode
{
public:
    Nav2ClientRobot2(const std::string& name, const BT::NodeConfiguration& config)
    ...
    {
        return{ BT::InputPort<Pose2D_2>("goal")};
    }

    virtual BT::NodeStatus tick() override
    {
        node_ = rclcpp::Node::make_shared("nav2_client");
        auto action_client = rclcpp_action::create_client<nav2_msgs::action::NavigateToPose>(node_, "robot2/navigate_to_pose");
        ...
        Pose2D_2 goal;
        if (!getInput<Pose2D_2>("goal", goal)) {
