...
class Camera_subscriber(Node):
    ...
    def __init__(self):
        ...
        # Dataloader
        view_img = check_imshow()  # Check if environment supports image displays # 检测cv2.imshow()方法是否可以执行，不能执行则抛出异常
    ...
    def camera_callback(self, data: Image):
        ...
        img = img.half() if self.half else img.float()  # uint8 to fp16/32 # 把输入从整型转化为半精度/全精度浮点数。
